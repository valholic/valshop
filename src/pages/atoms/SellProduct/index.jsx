...
export default function SellProduct() {
    ...
    function onSubmit(e) {
        ...
        data.append('seller_id', params.uid);
        data.append('seller_name', sellerName);
        image.map(img => {
            return data.append('product_images', img);
        })

        axios.post(`https://val-shops.vercel.app/v1/product/add`, data, {
            headers: {
    ...
    useEffect(() => {
        ...
        .then(result => {
            setSellerName(result.data.data.name);
        })
        .catch(err => {
            console.log(err);
        });
            ...
            <div className="w-3/5 h-auto p-5 m-auto bg-cyan-100">
                <form className="flex flex-wrap w-4/5 m-auto gap-y-6" onSubmit={onSubmit}>
                    ...
                    <fieldset className="flex flex-wrap w-full gap-y-3">
                        ...
                        <InputSelect name={'type'} values={['Cosmetic', 'Decor', 'Drink', 'Electronic', 'Fashion', 'Food', 'Furniture', 'Health', 'Sport']} label={"What's your product type ?"} handleChange={setType} />
                    </fieldset>
                    <fieldset>
                        <InputImage name={'product'} label={'Set your product sample!'} multiple={true} onChange={(e) => {
                                handleAddImage(e);
                            }} />
                        <div className="flex flex-wrap w-full h-auto gap-2 p-2 mt-4 rounded-md justify-evenly bg-slate-300">
                    ...
                    <Button type={'submit'} buttonName={'Add'} addSize={'w-full h-8'} />
