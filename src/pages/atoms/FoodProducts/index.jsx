...
export default function FoodProducts() {
    ...
    useEffect(() => {
        axios.get(`https://val-shops.vercel.app/v1/product/getfoo?page=${currentPage}&perPage=${perPage}`)
        .then(result => {
            ...
            <div className="w-full h-auto bg-slate-100">
                <div className="flex flex-wrap items-center w-4/5 h-auto gap-3 m-auto bg-white justify-evenly">
                    ...
                    {products.map(product => {
                        return <Card key={product._id} name={product.name} stock={product.stock} price={product.price} img={product.image} pid={product._id} />
                    })}
